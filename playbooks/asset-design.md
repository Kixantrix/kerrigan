...
2. **Golden-render regression.** Commit one approved reference render per asset — `previs/golden/<asset>.png` for 2D/voxel, a fixed-view PNG plus the DXF/STEP for CAD. The smoke test re-renders from source and diffs against the golden (perceptual/pixel diff within tolerance for raster; geometry/DXF diff for CAD). This both **forces a real render to exist** and catches silent drift. Update a golden only via a reviewed change with the new render attached and signed off.
