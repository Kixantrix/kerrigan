...
9. **Triage the mobile inbox.** At the start of a desktop session, scan `is:open label:agent:wait no:assignee` — these are ideas the human captured from phone via the `Mobile capture` issue template. For each: (a) refine into a briefing if worth doing now, (b) flip `agent:wait` → `agent:go` + assign Copilot, OR (c) close with a one-line reason, OR (d) leave as-is if it's a real "later" item. Don't let the inbox accumulate beyond ~10 — that means triage is overdue.
