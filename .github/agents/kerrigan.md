...
- **Per-repo merge model is declarative.** Each repo's branch-protection + merge-queue shape lives in `.github/repo-protection.json` (optimized default: `preset/kerrigan/repo-protection.json`). `tools/configure-repo-protection.ps1` applies it (dry-run by default; `-Apply` mutates shared protection — needs explicit human OK). `tools/validators/check_merge_queue.py` enforces the "every required check runs on `merge_group`" invariant so a merge queue can't silently stall. See [playbooks/repo-protection.md](../../playbooks/repo-protection.md).
